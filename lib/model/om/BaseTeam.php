...
abstract class BaseTeam extends BaseObject  implements Persistent
{
	...
	/**
	 * @var        array TeamI18n[] Collection to store aggregation of TeamI18n objects.
	 */
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = TeamPeer::SLUG;
		}
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = TeamPeer::TYPE;
		}
	...
	public function setLink($v)
	{
		...
		if ($this->link !== $v) {
			$this->link = $v;
			$this->modifiedColumns[] = TeamPeer::LINK;
		}
	...
	public function setPhoto($v)
	{
		...
		if ($this->photo !== $v) {
			$this->photo = $v;
			$this->modifiedColumns[] = TeamPeer::PHOTO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = TeamPeer::NUM_COLUMNS - TeamPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTeam:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTeam:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTeam:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(TeamPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTeam:save:post') as $callable)
				{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TeamPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TeamPeer::SLUG)) $criteria->add(TeamPeer::SLUG, $this->slug);
		if ($this->isColumnModified(TeamPeer::TYPE)) $criteria->add(TeamPeer::TYPE, $this->type);
		if ($this->isColumnModified(TeamPeer::NAME)) $criteria->add(TeamPeer::NAME, $this->name);
		if ($this->isColumnModified(TeamPeer::LINK)) $criteria->add(TeamPeer::LINK, $this->link);
		if ($this->isColumnModified(TeamPeer::PHOTO)) $criteria->add(TeamPeer::PHOTO, $this->photo);
	 ...
	 * @return     void
	 */
	public function initTeamI18ns()
	{
		$this->collTeamI18ns = new PropelObjectCollection();
		$this->collTeamI18ns->setModel('TeamI18n');
	}

	/**
	 * Gets an array of TeamI18n objects which contain a foreign key that references this object.
	 *
	 ...
	 * @return     PropelCollection|array TeamI18n[] List of TeamI18n objects
	 * @throws     PropelException
	 */
	public function getTeamI18ns($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collTeamI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collTeamI18ns) {
				// return empty collection
				$this->initTeamI18ns();
			} else {
				$collTeamI18ns = TeamI18nQuery::create(null, $criteria)
					->filterByTeam($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 * @throws     PropelException
	 */
	public function countTeamI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collTeamI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collTeamI18ns) {
				return 0;
			} else {
				$query = TeamI18nQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
	...
	/**
	 * Method called to associate a TeamI18n object to this object
	 * through the TeamI18n foreign key attribute.
	 *
	 * @param      TeamI18n $l TeamI18n
	 * @return     void
	 * @throws     PropelException
	 */
	public function addTeamI18n(TeamI18n $l)
	{
		if ($this->collTeamI18ns === null) {
			$this->initTeamI18ns();
		}
		if (!$this->collTeamI18ns->contains($l)) { // only add it if the **same** object is not already associated
			$this->collTeamI18ns[]= $l;
			$l->setTeam($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collTeamI18ns) {
				foreach ((array) $this->collTeamI18ns as $o) {
	...
	/**
	 * Returns the "about" value from the current {@link TeamI18n}.
	 */
	public function getAbout($culture = null)
	{
	  return $this->getCurrentTeamI18n($culture)->getAbout();
	}
	
	/**
	 * Sets the "about" value of the current {@link TeamI18n}.
	 *
	 * @return Team
	 */
	public function setAbout($value, $culture = null)
	{
	  $this->getCurrentTeamI18n($culture)->setAbout($value);
	  return $this;
	}
	 ...
	 * @return TeamI18n
	 */
	public function getCurrentTeamI18n($culture = null)
	{
	  if (null === $culture)
	  {
	  ...
	  {
	    $object = $this->isNew() ? null : TeamI18nPeer::retrieveByPK($this->getPrimaryKey(), $culture);
	    if ($object)
	    {
	      $this->setTeamI18nForCulture($object, $culture);
	    }
	    else
	    {
	      $this->setTeamI18nForCulture(new TeamI18n(), $culture);
	 ...
	 */
	public function setTeamI18nForCulture(TeamI18n $object, $culture)
	{
	  $this->current_i18n[$culture] = $object;
	  $this->addTeamI18n($object);
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseTeam:' . $name))
		{
