...
abstract class BaseGeneralInfo extends BaseObject  implements Persistent {
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				GeneralInfoPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
	...
	public function getVessel(PropelPDO $con = null)
	{
		if ($this->aVessel === null && ($this->vessel_id !== null)) {
			$this->aVessel = VesselPeer::retrieveByPk($this->vessel_id);
	...
	public function getSkipper(PropelPDO $con = null)
	{
		if ($this->aSkipper === null && ($this->skipper_id !== null)) {
			$this->aSkipper = SkipperPeer::retrieveByPk($this->skipper_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
	...
	public function getGuide(PropelPDO $con = null)
	{
		if ($this->aGuide === null && ($this->guide_id !== null)) {
			$this->aGuide = GuidePeer::retrieveByPk($this->guide_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
	 ...
	 * Otherwise if this GeneralInfo has previously been saved, it will retrieve
	 * related Records from storage. If this GeneralInfo is new, it will return
		...
		if ($this->collRecords === null) {
			if ($this->isNew()) {
			   $this->collRecords = array();
				...
				$criteria->add(RecordPeer::GENERAL_INFO_ID, $this->id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseGeneralInfo:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseGeneralInfo::%s', $method));
	  }
