...
/**
 * Base class that represents a row from the 'profile_i18n' table.
 *
 * 
 *
...
abstract class BaseProfileI18n extends BaseObject  implements Persistent
{
  ...
  const PEER = 'ProfileI18nPeer';
	 ...
	 * @return     ProfileI18n The current object (for fluent API support)
	 */
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = ProfileI18nPeer::TYPE;
		}
	...
	public function setAbout($v)
	{
		...
		if ($this->about !== $v) {
			$this->about = $v;
			$this->modifiedColumns[] = ProfileI18nPeer::ABOUT;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = ProfileI18nPeer::NUM_COLUMNS - ProfileI18nPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ProfileI18nPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseProfileI18n:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ProfileI18nQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseProfileI18n:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseProfileI18n:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseProfileI18n:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				ProfileI18nPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aProfile !== null) {
				if ($this->aProfile->isModified() || ($this->aProfile->getCulture() && $this->aProfile->getCurrentProfileI18n()->isModified()) || $this->aProfile->isNew()) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aProfile) {
				$result['Profile'] = $this->aProfile->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProfileI18nPeer::TYPE)) $criteria->add(ProfileI18nPeer::TYPE, $this->type);
		if ($this->isColumnModified(ProfileI18nPeer::ABOUT)) $criteria->add(ProfileI18nPeer::ABOUT, $this->about);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ProfileI18n (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ProfileI18n Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getProfile(PropelPDO $con = null)
	{
		if ($this->aProfile === null && ($this->id !== null)) {
			$this->aProfile = ProfileQuery::create()->findPk($this->id);
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseProfileI18n:' . $name))
		{
