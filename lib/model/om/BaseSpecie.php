...
abstract class BaseSpecie extends BaseObject  implements Persistent {
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collSightings.
	 */
	private $lastSightingCriteria = null;
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SpeciePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = SpeciePeer::NUM_COLUMNS - SpeciePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastSightingCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SpeciePeer::doInsert($this, $con);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SpeciePeer::NAME)) $criteria->add(SpeciePeer::NAME, $this->name);
	...
	public function getSpecieGroup(PropelPDO $con = null)
	{
		if ($this->aSpecieGroup === null && ($this->specie_group_id !== null)) {
			$this->aSpecieGroup = SpecieGroupPeer::retrieveByPk($this->specie_group_id);
	...
	/**
	 * Clears out the collSightings collection (array).
	 *
	...
	/**
	 * Initializes the collSightings collection (array).
	 *
	 ...
	 * Otherwise if this Specie has previously been saved, it will retrieve
	 * related Sightings from storage. If this Specie is new, it will return
		...
		if ($this->collSightings === null) {
			if ($this->isNew()) {
			   $this->collSightings = array();
				...
				$criteria->add(SightingPeer::SPECIE_ID, $this->id);

				SightingPeer::addSelectColumns($criteria);
				$this->collSightings = SightingPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				SightingPeer::addSelectColumns($criteria);
				if (!isset($this->lastSightingCriteria) || !$this->lastSightingCriteria->equals($criteria)) {
					$this->collSightings = SightingPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSightingCriteria = $criteria;
	...
	public function countSightings(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collSightings === null) {
			if ($this->isNew()) {
				...
				$count = SightingPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastSightingCriteria) || !$this->lastSightingCriteria->equals($criteria)) {
					...
					$count = count($this->collSightings);
				}
			} else {
				$count = count($this->collSightings);
			}
	...
	public function addSighting(Sighting $l)
	{
		...
		if (!in_array($l, $this->collSightings, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSightings, $l);
			$l->setSpecie($this);
		}
	}
	...
	public function getSightingsJoinRecord($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collSightings === null) {
			if ($this->isNew()) {
				...
				$this->collSightings = SightingPeer::doSelectJoinRecord($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function getSightingsJoinBehaviour($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collSightings === null) {
			if ($this->isNew()) {
				...
				$this->collSightings = SightingPeer::doSelectJoinBehaviour($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function getSightingsJoinAssociation($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collSightings === null) {
			if ($this->isNew()) {
				...
				$this->collSightings = SightingPeer::doSelectJoinAssociation($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseSpecie:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseSpecie::%s', $method));
	  }
