...
abstract class BaseCompany extends BaseObject  implements Persistent {
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collGuides.
	 */
	private $lastGuideCriteria = null;
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collSkippers.
	 */
	private $lastSkipperCriteria = null;
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collVessels.
	 */
	private $lastVesselCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->base_latitude = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->base_longitude = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 6; // 6 = CompanyPeer::NUM_COLUMNS - CompanyPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastGuideCriteria = null;

			$this->collSkippers = null;
			$this->lastSkipperCriteria = null;

			$this->collVessels = null;
			$this->lastVesselCriteria = null;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
			$keys[2] => $this->getBaseLatitude(),
			$keys[3] => $this->getBaseLongitude(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setBaseLatitude($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setBaseLongitude($arr[$keys[3]]);
	...
	/**
	 * Clears out the collGuides collection (array).
	 *
	...
	/**
	 * Initializes the collGuides collection (array).
	 *
	 ...
	 * related Guides from storage. If this Company is new, it will return
		...
		if ($this->collGuides === null) {
			if ($this->isNew()) {
			   $this->collGuides = array();
				...
				$criteria->add(GuidePeer::COMPANY_ID, $this->id);

				GuidePeer::addSelectColumns($criteria);
				$this->collGuides = GuidePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				GuidePeer::addSelectColumns($criteria);
				if (!isset($this->lastGuideCriteria) || !$this->lastGuideCriteria->equals($criteria)) {
					$this->collGuides = GuidePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastGuideCriteria = $criteria;
	...
	public function countGuides(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collGuides === null) {
			if ($this->isNew()) {
				...
				$count = GuidePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addGuide(Guide $l)
	{
		...
		if (!in_array($l, $this->collGuides, true)) { // only add it if the **same** object is not already associated
			array_push($this->collGuides, $l);
			$l->setCompany($this);
		}
	}

	/**
	 * Clears out the collSkippers collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collSkippers collection (array).
	 *
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Company has previously been saved, it will retrieve
	 * related Skippers from storage. If this Company is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array Skipper[]
	 * @throws     PropelException
	 */
		...
		if ($this->collSkippers === null) {
			if ($this->isNew()) {
			   $this->collSkippers = array();
				...
				$criteria->add(SkipperPeer::COMPANY_ID, $this->id);

				SkipperPeer::addSelectColumns($criteria);
				$this->collSkippers = SkipperPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				SkipperPeer::addSelectColumns($criteria);
				if (!isset($this->lastSkipperCriteria) || !$this->lastSkipperCriteria->equals($criteria)) {
					$this->collSkippers = SkipperPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastSkipperCriteria = $criteria;
	...
	public function countSkippers(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collSkippers === null) {
			if ($this->isNew()) {
				...
				$count = SkipperPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addSkipper(Skipper $l)
	{
		...
		if (!in_array($l, $this->collSkippers, true)) { // only add it if the **same** object is not already associated
			array_push($this->collSkippers, $l);
			$l->setCompany($this);
		}
	}

	/**
	 * Clears out the collVessels collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collVessels collection (array).
	 *
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Company has previously been saved, it will retrieve
	 * related Vessels from storage. If this Company is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collVessels === null) {
			if ($this->isNew()) {
			   $this->collVessels = array();
				...
				$criteria->add(VesselPeer::COMPANY_ID, $this->id);

				VesselPeer::addSelectColumns($criteria);
				$this->collVessels = VesselPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				VesselPeer::addSelectColumns($criteria);
				if (!isset($this->lastVesselCriteria) || !$this->lastVesselCriteria->equals($criteria)) {
					$this->collVessels = VesselPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastVesselCriteria = $criteria;
	...
	public function countVessels(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collVessels === null) {
			if ($this->isNew()) {
				...
				$count = VesselPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addVessel(Vessel $l)
	{
		...
		if (!in_array($l, $this->collVessels, true)) { // only add it if the **same** object is not already associated
			array_push($this->collVessels, $l);
			$l->setCompany($this);
		}
	}
