...
abstract class BasePhoto extends BaseObject  implements Persistent
{
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = PhotoPeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->album_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(PhotoPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PhotoPeer::SLUG)) $criteria->add(PhotoPeer::SLUG, $this->slug);
	...
	/**
	 * Clears out the collPhotoI18ns collection
	 *
	...
	/**
	 * Initializes the collPhotoI18ns collection.
	 *
	...
	{
		$this->collPhotoI18ns = new PropelObjectCollection();
		$this->collPhotoI18ns->setModel('PhotoI18n');
	}
	 ...
	 * @return     PropelCollection|array PhotoI18n[] List of PhotoI18n objects
	 * @throws     PropelException
	 */
	public function getPhotoI18ns($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collPhotoI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collPhotoI18ns) {
				// return empty collection
				$this->initPhotoI18ns();
			} else {
				$collPhotoI18ns = PhotoI18nQuery::create(null, $criteria)
					->filterByPhoto($this)
					->find($con);
				if (null !== $criteria) {
					return $collPhotoI18ns;
				}
				$this->collPhotoI18ns = $collPhotoI18ns;
			}
		}
	...
	public function countPhotoI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collPhotoI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collPhotoI18ns) {
				...
				$query = PhotoI18nQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collPhotoI18ns);
		}
	}
	...
	public function addPhotoI18n(PhotoI18n $l)
	{
		...
		if (!$this->collPhotoI18ns->contains($l)) { // only add it if the **same** object is not already associated
			$this->collPhotoI18ns[]= $l;
			$l->setPhoto($this);
		}
	}
	...
	/**
	 * Returns the "caption" value from the current {@link PhotoI18n}.
	 */
	public function getCaption($culture = null)
	{
	  return $this->getCurrentPhotoI18n($culture)->getCaption();
	}
	
	/**
	 * Sets the "caption" value of the current {@link PhotoI18n}.
	 *
	 * @return Photo
	 */
	public function setCaption($value, $culture = null)
	{
	  $this->getCurrentPhotoI18n($culture)->setCaption($value);
	  return $this;
	}
