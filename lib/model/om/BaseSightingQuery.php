 ...
 * @method     SightingQuery orderByNumberVessels($order = Criteria::ASC) Order by the number_vessels column
 ...
 * @method     SightingQuery groupByNumberVessels() Group by the number_vessels column
 ...
 * @method     Sighting findOneByCubs(string $cubs) Return the first Sighting filtered by the cubs column
 * @method     Sighting findOneByTotal(int $total) Return the first Sighting filtered by the total column
 * @method     Sighting findOneByNumberVessels(int $number_vessels) Return the first Sighting filtered by the number_vessels column
 ...
 * @method     array findByCubs(string $cubs) Return Sighting objects filtered by the cubs column
 * @method     array findByTotal(int $total) Return Sighting objects filtered by the total column
 * @method     array findByNumberVessels(int $number_vessels) Return Sighting objects filtered by the number_vessels column
...
abstract class BaseSightingQuery extends ModelCriteria
{
	...
	{
		if(preg_match('/[\%\*]/', $adults)) {
			return $this->addUsingAlias(SightingPeer::ADULTS, str_replace('*', '%', $adults), Criteria::LIKE);
		} else {
	...
	{
		if(preg_match('/[\%\*]/', $juveniles)) {
			return $this->addUsingAlias(SightingPeer::JUVENILES, str_replace('*', '%', $juveniles), Criteria::LIKE);
		} else {
	 ...
	 * 
	 * @param     string $cubs The value to use as filter.
	 *            Accepts wildcards (* and % trigger a LIKE)
	 *
	 * @return    SightingQuery The current query, for fluid interface
	 */
	...
	{
		if(preg_match('/[\%\*]/', $cubs)) {
			return $this->addUsingAlias(SightingPeer::CUBS, str_replace('*', '%', $cubs), Criteria::LIKE);
		} else {
	...
	/**
	 * Filter the query on the number_vessels column
	 * 
	 * @param     int|array $number_vessels The value to use as filter.
	 *            Accepts an associative array('min' => $minValue, 'max' => $maxValue)
	 *
	 * @return    SightingQuery The current query, for fluid interface
	 */
	public function filterByNumberVessels($numberVessels = null)
	{
		if (is_array($numberVessels)) {
			if (array_values($numberVessels) === $numberVessels) {
				return $this->addUsingAlias(SightingPeer::NUMBER_VESSELS, $numberVessels, Criteria::IN);
			} else {
				if (isset($numberVessels['min'])) {
					$this->addUsingAlias(SightingPeer::NUMBER_VESSELS, $numberVessels['min'], Criteria::GREATER_EQUAL);
				}
				if (isset($numberVessels['max'])) {
					$this->addUsingAlias(SightingPeer::NUMBER_VESSELS, $numberVessels['max'], Criteria::LESS_EQUAL);
				}
				return $this;	
			}
		} else {
			return $this->addUsingAlias(SightingPeer::NUMBER_VESSELS, $numberVessels, Criteria::EQUAL);
		}
	}
	...
	public function filterByComments($comments = null)
	{
		if(preg_match('/[\%\*]/', $comments)) {
			return $this->addUsingAlias(SightingPeer::COMMENTS, str_replace('*', '%', $comments), Criteria::LIKE);
		} else {
			return $this->addUsingAlias(SightingPeer::COMMENTS, $comments, Criteria::EQUAL);
		}
	}
