...
/**
 * Base class that represents a row from the 'guide' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseGuide extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'GuidePeer';
	 ...
	 * @return     Guide The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = GuidePeer::ID;
		}
	...
	public function setCompanyId($v)
	{
		...
		if ($this->company_id !== $v) {
			$this->company_id = $v;
			$this->modifiedColumns[] = GuidePeer::COMPANY_ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = GuidePeer::NAME;
		}
	...
	public function setObservations($v)
	{
		...
		if ($this->observations !== $v) {
			$this->observations = $v;
			$this->modifiedColumns[] = GuidePeer::OBSERVATIONS;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = GuidePeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = GuidePeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = GuidePeer::NUM_COLUMNS - GuidePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Guide object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GuidePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseGuide:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGuide:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseGuide:save:pre') as $callable)
			{
			...
			if ($this->isModified() && !$this->isColumnModified(GuidePeer::UPDATED_AT))
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(GuidePeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGuide:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				GuidePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(GuidePeer::ID) ) {
					...
					$affectedRows += GuidePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = GuidePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = GuidePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = GuidePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GuidePeer::ID)) $criteria->add(GuidePeer::ID, $this->id);
		if ($this->isColumnModified(GuidePeer::COMPANY_ID)) $criteria->add(GuidePeer::COMPANY_ID, $this->company_id);
		if ($this->isColumnModified(GuidePeer::NAME)) $criteria->add(GuidePeer::NAME, $this->name);
		if ($this->isColumnModified(GuidePeer::OBSERVATIONS)) $criteria->add(GuidePeer::OBSERVATIONS, $this->observations);
		if ($this->isColumnModified(GuidePeer::CREATED_AT)) $criteria->add(GuidePeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(GuidePeer::UPDATED_AT)) $criteria->add(GuidePeer::UPDATED_AT, $this->updated_at);
	...
	{
		$criteria = new Criteria(GuidePeer::DATABASE_NAME);
		$criteria->add(GuidePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Guide (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Guide Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Guide is new, it will return
	 ...
	 * Otherwise if this Guide is new, it will return
	 * an empty collection; or if this Guide has previously
	 * been saved, it will retrieve related GeneralInfos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Guide.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseGuide:' . $name))
		{
