...
/**
 * Base class that represents a row from the 'direction' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseDirection extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'DirectionPeer';
	 ...
	 * @return     Direction The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = DirectionPeer::ID;
		}
	...
	public function setAcronym($v)
	{
		...
		if ($this->acronym !== $v) {
			$this->acronym = $v;
			$this->modifiedColumns[] = DirectionPeer::ACRONYM;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = DirectionPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = DirectionPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = DirectionPeer::NUM_COLUMNS - DirectionPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Direction object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DirectionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseDirection:delete:pre') as $callable)
			{
			...
			if ($ret) {
				DirectionQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseDirection:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseDirection:save:pre') as $callable)
			{
			...
			if ($this->isModified() && !$this->isColumnModified(DirectionPeer::UPDATED_AT))
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(DirectionPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseDirection:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				DirectionPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(DirectionPeer::ID) ) {
					...
					$affectedRows = DirectionPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = DirectionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = DirectionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = DirectionPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(DirectionPeer::ID)) $criteria->add(DirectionPeer::ID, $this->id);
		if ($this->isColumnModified(DirectionPeer::ACRONYM)) $criteria->add(DirectionPeer::ACRONYM, $this->acronym);
		if ($this->isColumnModified(DirectionPeer::CREATED_AT)) $criteria->add(DirectionPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(DirectionPeer::UPDATED_AT)) $criteria->add(DirectionPeer::UPDATED_AT, $this->updated_at);
	...
	{
		$criteria = new Criteria(DirectionPeer::DATABASE_NAME);
		$criteria->add(DirectionPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Direction (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Direction Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     DirectionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new DirectionPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this Direction is new, it will return
	 ...
	 * Otherwise if this Direction is new, it will return
	 * an empty collection; or if this Direction has previously
	 * been saved, it will retrieve related WatchSightings from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Direction.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseDirection:' . $name))
		{
