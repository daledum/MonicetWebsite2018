...
/**
 * Base class that represents a row from the 'watchman' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseWatchman extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'WatchmanPeer';
	 ...
	 * @return     Watchman The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = WatchmanPeer::ID;
		}
	...
	public function setCompanyId($v)
	{
		...
		if ($this->company_id !== $v) {
			$this->company_id = $v;
			$this->modifiedColumns[] = WatchmanPeer::COMPANY_ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = WatchmanPeer::NAME;
		}
	...
	public function setObservations($v)
	{
		...
		if ($this->observations !== $v) {
			$this->observations = $v;
			$this->modifiedColumns[] = WatchmanPeer::OBSERVATIONS;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = WatchmanPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = WatchmanPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->observations = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 6; // 6 = WatchmanPeer::NUM_COLUMNS - WatchmanPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Watchman object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = WatchmanPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseWatchman:delete:pre') as $callable)
			{
			...
			if ($ret) {
				WatchmanQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseWatchman:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseWatchman:save:pre') as $callable)
			{
			...
			if ($this->isModified() && !$this->isColumnModified(WatchmanPeer::UPDATED_AT))
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(WatchmanPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseWatchman:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				WatchmanPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(WatchmanPeer::ID) ) {
					...
					$affectedRows += WatchmanPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = WatchmanPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = WatchmanPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = WatchmanPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getCompanyId(),
			$keys[2] => $this->getName(),
			$keys[3] => $this->getObservations(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setObservations($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(WatchmanPeer::ID)) $criteria->add(WatchmanPeer::ID, $this->id);
		if ($this->isColumnModified(WatchmanPeer::COMPANY_ID)) $criteria->add(WatchmanPeer::COMPANY_ID, $this->company_id);
		if ($this->isColumnModified(WatchmanPeer::NAME)) $criteria->add(WatchmanPeer::NAME, $this->name);
		if ($this->isColumnModified(WatchmanPeer::OBSERVATIONS)) $criteria->add(WatchmanPeer::OBSERVATIONS, $this->observations);
		if ($this->isColumnModified(WatchmanPeer::CREATED_AT)) $criteria->add(WatchmanPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(WatchmanPeer::UPDATED_AT)) $criteria->add(WatchmanPeer::UPDATED_AT, $this->updated_at);
	...
	{
		$criteria = new Criteria(WatchmanPeer::DATABASE_NAME);
		$criteria->add(WatchmanPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Watchman (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Watchman Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getCompany(PropelPDO $con = null)
	{
		if ($this->aCompany === null && ($this->company_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 $this->aCompany->addWatchmans($this);
			 */
		}
	 ...
	 * If this Watchman is new, it will return
	 ...
	 * Otherwise if this Watchman is new, it will return
	 * an empty collection; or if this Watchman has previously
	 * been saved, it will retrieve related WatchInfos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Watchman.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseWatchman:' . $name))
		{
