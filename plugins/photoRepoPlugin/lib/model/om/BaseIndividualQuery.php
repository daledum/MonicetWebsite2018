 ...
 * @method     IndividualQuery orderByDominantBodyPartCode($order = Criteria::ASC) Order by the dominant_body_part_code column
 ...
 * @method     IndividualQuery groupByDominantBodyPartCode() Group by the dominant_body_part_code column
 ...
 * @method     Individual findOneByDominantBodyPartCode(string $dominant_body_part_code) Return the first Individual filtered by the dominant_body_part_code column
 ...
 * @method     array findByDominantBodyPartCode(string $dominant_body_part_code) Return Individual objects filtered by the dominant_body_part_code column
...
abstract class BaseIndividualQuery extends ModelCriteria
{
	...
	/**
	 * Filter the query on the dominant_body_part_code column
	 * 
	 * @param     string $dominantBodyPartCode The value to use as filter.
	 *            Accepts wildcards (* and % trigger a LIKE)
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    IndividualQuery The current query, for fluid interface
	 */
	public function filterByDominantBodyPartCode($dominantBodyPartCode = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($dominantBodyPartCode)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $dominantBodyPartCode)) {
				$dominantBodyPartCode = str_replace('*', '%', $dominantBodyPartCode);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(IndividualPeer::DOMINANT_BODY_PART_CODE, $dominantBodyPartCode, $comparison);
	}
