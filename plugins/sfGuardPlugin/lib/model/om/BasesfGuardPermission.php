...
abstract class BasesfGuardPermission extends BaseObject  implements Persistent
{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collmfMenus !== null) {
				foreach ($this->collmfMenus as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getmfMenus() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addmfMenu($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collmfMenus collection
	 *
	 ...
	 * @see        addmfMenus()
	 */
	public function clearmfMenus()
	{
		$this->collmfMenus = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collmfMenus collection.
	 *
	 * By default this just sets the collmfMenus collection to an empty array (like clearcollmfMenus());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initmfMenus()
	{
		$this->collmfMenus = new PropelObjectCollection();
		$this->collmfMenus->setModel('mfMenu');
	}
	 ...
	 * @throws     PropelException
	 */
	public function getmfMenus($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collmfMenus || null !== $criteria) {
			if ($this->isNew() && null === $this->collmfMenus) {
				// return empty collection
				$this->initmfMenus();
			} else {
				$collmfMenus = mfMenuQuery::create(null, $criteria)
					->filterBysfGuardPermission($this)
					->find($con);
				if (null !== $criteria) {
					return $collmfMenus;
				}
				$this->collmfMenus = $collmfMenus;
			}
		}
		return $this->collmfMenus;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countmfMenus(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collmfMenus || null !== $criteria) {
			if ($this->isNew() && null === $this->collmfMenus) {
				return 0;
	 ...
	 * @throws     PropelException
	 */
	public function addmfMenu(mfMenu $l)
	{
		if ($this->collmfMenus === null) {
			$this->initmfMenus();
		}
		if (!$this->collmfMenus->contains($l)) { // only add it if the **same** object is not already associated
			$this->collmfMenus[]= $l;
	 ...
	 * been saved, it will retrieve related mfMenus from storage.
	 *
	 ...
	 * @return     PropelCollection|array mfMenu[] List of mfMenu objects
	 */
	public function getmfMenusJoinmfMenuRelatedByMenuPaiId($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = mfMenuQuery::create(null, $criteria);
		$query->joinWith('mfMenuRelatedByMenuPaiId', $join_behavior);

		return $this->getmfMenus($query, $con);
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collmfMenus) {
				foreach ((array) $this->collmfMenus as $o) {
